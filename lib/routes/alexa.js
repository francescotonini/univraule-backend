...
let processDynamicResponses = (rooms) => {
	...
	apl.datasources.listTemplate1ListData.listPage = [];
	if (rooms['length'] != 0) {
		...
		rooms.forEach((room) => {
			...
			apl.datasources.listTemplate1ListData.listPage.push({
				listItemIdentifier: room['name'],
				ordinalNumber: 2,
				textContent: {
					primaryText: {
						type: 'PlainText',
