...
const getCourses = async () => {
	...
	if (!academicYears['length']) {
		console.log('No academic years found. You should have a look');
...
const getWeeklyTimetable = async (firstDayOfWeek, academicYearId, courseId, courseYearId) => {
	...
	if (response['statusCode'] != 200) {
		console.log(`Invalid response from ${POST_TIMETABLE_ENDPOINT} (${response['statusCode']}): ${response['body']}`);
	...
	try {
		...
		console.log(`Unable to parse timetable: ${e['message']}`);
...
const getOffices = async () => {
	...
	try {
		...
		console.log(`Unable to parse offices response ${e['message']}`);

		return [];
	}

	if (!offices['length']) {
		console.log('No offices years found. You should have a look');
...
const getRooms = async (officeId) => {
	...
	if (!parsedResponse['events'] || !parsedResponse['order'] || !parsedResponse['area_rooms']) {
		console.log('Unable to get rooms, response is invalid');
	...
	roomsId.forEach((roomId) => {
		...
		if (!roomData) {
			console.log(`Room data for id ${roomId} not found`);
			return;
		}
