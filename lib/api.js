...
const getLessons = async (month, year, academicYearId, courseId, courseYearId) => {
	...
	let lessons = [];
	let lastDayOfMonth = moment(`${month} ${year}`, 'MM YYYY').tz('Europe/Rome').endOf('month');
	let currentDay = moment(`${month} ${year}`, 'MM YYYY').tz('Europe/Rome').startOf('month');
...
const getRooms = async (officeId) => {
	...
	roomsId.forEach((roomId) => {
		...
		if (!currentEvent && nextEvent) {
			...
			room['isFree'] = true;
			room['until'] = closingTime.getTime();
		}
