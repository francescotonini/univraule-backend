...
const alexa = {
	// POST /alexa
	// TODO: rename to /alexa/vr
	getUniVR: (req, res, next) => {
		...
		if (staticResponse) {
			...
			async.map(officeIds, 'vr', (officeId, cb) => {
	...
	// POST /alexa/tn
	getUniTN: (req, res, next) => {
		const officeIds = ['E0503'];
		...
		if (staticResponse) {
			...
			logger.info('Received a \'aule libere\' intent for unitn from Alexa');

			async.map(officeIds, 'tn', (officeId, cb) => {
				data.getRooms(officeId, cb);
			}, (err, arrayOfArrayOfRooms) => {
				if (err) {
					cb && cb(err);
