...
const getCourses = async () => {
	...
	try {
		...
		logger.error(`getCourses(): exception on request - ${e['message']}`);
...
const getLessons = async (month, year, academicYearId, courseId, courseYearId) => {
	...
	while (currentDay.date() != lastDayOfMonth.date()) {
		...
		if (timetable['celle']) {
			timetable['celle'].forEach((e) => {
				...
				let startDateRaw = `${year}-${month}-${currentDay.date() + offsetFromFirstDayOfWeek} ${e['ora_inizio'].split(':')[0]}:${e['ora_inizio'].split(':')[1]}`;
				let endDateRaw = `${year}-${month}-${currentDay.date() + offsetFromFirstDayOfWeek} ${e['ora_fine'].split(':')[0]}:${e['ora_fine'].split(':')[1]}`;
	...
	logger.debug(`getLessons(): found ${lessons['length']} lessons`);
...
const getWeeklyTimetable = async (firstDayOfWeek, academicYearId, courseId, courseYearId) => {
	...
	try {
		...
		logger.error(`getWeeklyTimetable(): exception on request - ${e['message']}`);

		return [];
	}

	if (response['statusCode'] != 200) {
		logger.error(`getWeeklyTimetable(): invalid response from ${POST_TIMETABLE_ENDPOINT} (${response['statusCode']}): ${response['body']}`);
	...
	try {
		...
		logger.error(`getWeeklyTimetable(): unable to parse timetable: ${e['message']}`);
...
const getOffices = async () => {
	...
	try {
		...
		logger.error(`getOffices(): exception on request - ${e['message']}`);
...
const getRooms = async (officeId) => {
	...
	try {
		...
		logger.error(`getRooms(): exception on request - ${e['message']}`);

		return [];
	}

	if (response['statusCode'] != 200) {
		logger.error(`getRooms(): invalid response from ${POST_TIMETABLE_ENDPOINT} (${response['statusCode']}): ${response['body']}`);
	...
	try {
		...
		logger.error(`getRooms(): unable to parse response: ${e['message']}`);
		
		return [];
	}

	if (!parsedResponse['events'] || !parsedResponse['order'] || !parsedResponse['area_rooms']) {
		logger.error('getRooms(): unable to get rooms, response is invalid');
	...
	logger.debug(`getRooms(): found ${rooms['length']} rooms`);
 ...
 * @param - course year id
 */
const getTeachings = async (academicYearId, courseId, courseYearId) => {
	let response;
	try {
		response = await phin({
		...
		logger.error(`getTeachings(): exception on request - ${e['message']}`);
